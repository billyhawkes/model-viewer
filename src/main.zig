...
export fn init() void {
    renderer = hr.Renderer.init();
    simgui.setup(.{
        .logger = .{ .func = slog.func },
    });
    ig.igGetIO().*.ConfigFlags |= ig.ImGuiConfigFlags_DockingEnable;

    renderer.submit(&state.model.mesh);
}
...
pub fn main() !void {
    state.model.mesh = try hr.Mesh.parse(allocator, "./mesh");
